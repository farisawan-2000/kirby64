#include <ultra64.h>
#include <macros.h>

#ifdef MIPS_TO_C
//generated by mips_to_c commit e0e006e8858ba357d1dcb4dc64f038b7df278aa6
s32 change_kirby_hp(f32 arg0) {
    f32 phi_f12;
    f32 phi_f0;

    phi_f12 = arg0;
    if ((func_80018AAC_ovl1() & 3) == 3) {
        phi_f12 = arg0;
        if (&func_800A6B64 == 0) {
            if (arg0 < 0.0f) {
                phi_f12 = arg0 * 2.0f;
            } else {
                phi_f12 = 0.0f;
            }
        }
    }
    if (0.0f == (f32) D_800D6E8C) {
        return 0;
    }
    gKirbyHp = (f32) (gKirbyHp + phi_f12);
    if (gKirbyHp <= 0.0f) {
        gKirbyHp = 0.0f;
    }
    phi_f0 = gKirbyHp;
    if (6.0f < gKirbyHp) {
        gKirbyHp = 6.0f;
        phi_f0 = gKirbyHp;
    }
    D_800D6E8C = (s32) phi_f0;
    D_800E7B20 = phi_f0;
    return D_800D6E8C;
}
#else
GLOBAL_ASM("asm/non_matchings/ovl1_13/ovl1_13/change_kirby_hp.s")
#endif

#ifdef MIPS_TO_C
//generated by mips_to_c commit e0e006e8858ba357d1dcb4dc64f038b7df278aa6
void func_800BC0F0(s32 arg0) {
    s32 phi_a0;
    s32 phi_a0_2;

    phi_a0 = arg0;
    if (arg0 < 0) {
        phi_a0 = 0;
    }
    phi_a0_2 = phi_a0;
    if (phi_a0 >= 3) {
        phi_a0_2 = 2;
    }
    D_800D6E90 = (s32) (phi_a0_2 + 0x23);
}
#else
GLOBAL_ASM("asm/non_matchings/ovl1_13/ovl1_13/func_800BC0F0.s")
#endif

#ifdef MIPS_TO_C
//generated by mips_to_c commit e0e006e8858ba357d1dcb4dc64f038b7df278aa6
s32 func_800BC11C(f32 arg0) {
    f32 temp_f12;
    f32 phi_f0;

    D_800D6E58 = arg0;
    if (D_800D6E58 < 0.0f) {
        D_800D6E58 = 0.0f;
    }
    phi_f0 = D_800D6E58;
    if (D_800D6E5C < D_800D6E58) {
        D_800D6E58 = (f32) D_800D6E5C;
        phi_f0 = D_800D6E58;
    }
    temp_f12 = (phi_f0 / D_800D6E5C) * 30.0f;
    if (D_800D6E5C == phi_f0) {
        D_800D6E94 = 0x1E;
    } else {
        if (0.0f == phi_f0) {
            D_800D6E94 = 0;
        } else {
            if (temp_f12 < 1.0f) {
                D_800D6E94 = 1;
            } else {
                D_800D6E94 = (s32) temp_f12;
            }
        }
    }
    return D_800D6E94;
}
#else
GLOBAL_ASM("asm/non_matchings/ovl1_13/ovl1_13/func_800BC11C.s")
#endif

#ifdef MIPS_TO_C
//generated by mips_to_c commit e0e006e8858ba357d1dcb4dc64f038b7df278aa6
void *func_800BC1FC(s32 arg0) {
    D_800D6E58 = (f32) arg0;
    D_800D6E5C = (f32) D_800D6E58;
    D_800D6EB8 = 0;
    D_800D6EAC = 0;
    D_800D6E94 = 0x1E;
    return &D_800D6E58;
}
#else
GLOBAL_ASM("asm/non_matchings/ovl1_13/ovl1_13/func_800BC1FC.s")
#endif

#ifdef MIPS_TO_C
//generated by mips_to_c commit e0e006e8858ba357d1dcb4dc64f038b7df278aa6
void *func_800BC240(s32 arg0) {
    D_800D6E60 = (s32) (D_800D6E60 + arg0);
    return &D_800D6E60;
}
#else
GLOBAL_ASM("asm/non_matchings/ovl1_13/ovl1_13/func_800BC240.s")
#endif

#ifdef MIPS_TO_C
//generated by mips_to_c commit e0e006e8858ba357d1dcb4dc64f038b7df278aa6
s32 func_800BC258(s32 arg0) {
    s32 temp_v0;
    s32 phi_v0;
    s32 phi_v0_2;
    s32 phi_return;

    temp_v0 = gKirbyLives + arg0;
    gKirbyLives = temp_v0;
    phi_v0 = temp_v0;
    phi_return = temp_v0;
    if (temp_v0 < 0) {
        gKirbyLives = 0;
        phi_v0 = 0;
        phi_return = 0;
    }
    phi_v0_2 = phi_v0;
    if (phi_v0 >= 0x65) {
        gKirbyLives = 0x64;
        phi_v0_2 = 0x64;
        phi_return = 0x64;
    }
    D_800D6E88 = phi_v0_2;
    return phi_return;
}
#else
GLOBAL_ASM("asm/non_matchings/ovl1_13/ovl1_13/func_800BC258.s")
#endif

#ifdef MIPS_TO_C
//generated by mips_to_c commit e0e006e8858ba357d1dcb4dc64f038b7df278aa6
void *func_800BC298(s32 arg0) {
    s32 temp_a0;
    s32 phi_a0;
    s32 phi_a0_2;
    s32 phi_a0_3;

    temp_a0 = arg0;
    phi_a0 = temp_a0;
    if ((func_80018AAC_ovl1() & 3) == 2) {
        arg0 = temp_a0;
        phi_a0 = arg0;
        if (func_800F88A0_ovl1(temp_a0) == 0) {
            D_800D6E54 = 0;
            D_800D6E90 = 0;
            D_8012E850 = 0;
            return &D_800D6E54;
        }
    }
    phi_a0_2 = phi_a0;
    if (phi_a0 < 0) {
        phi_a0_2 = 0;
    }
    phi_a0_3 = phi_a0_2;
    if (phi_a0_2 >= 0x24) {
        phi_a0_3 = 0x23;
    }
    D_800D6E90 = phi_a0_3;
    D_800D6E54 = phi_a0_3;
    return &D_800D6E54;
}
#else
GLOBAL_ASM("asm/non_matchings/ovl1_13/ovl1_13/func_800BC298.s")
#endif

#ifdef MIPS_TO_C
//generated by mips_to_c commit e0e006e8858ba357d1dcb4dc64f038b7df278aa6
void func_800BC328(s32 arg0) {
    f32 temp_f16;
    f32 temp_f6;
    s32 temp_t0;
    s32 temp_v0_2;
    void *temp_a3;
    void *temp_v0;
    f32 phi_f6;
    f32 phi_f16;
    s32 phi_v0;
    ? phi_a3;

    temp_t0 = D_8004A7C4->unk0 * 4;
    temp_a3 = &D_800E9E20 + temp_t0;
    temp_v0 = (*temp_a3 * 4) + &D_800D552C;
    temp_f6 = (f32) temp_v0->unk0;
    phi_f6 = temp_f6;
    if (temp_v0->unk0 < 0) {
        phi_f6 = temp_f6 + 4294967296.0f;
    }
    *(&D_800E25D0 + temp_t0) = phi_f6;
    temp_f16 = (f32) temp_v0->unk4;
    phi_f16 = temp_f16;
    if (temp_v0->unk4 < 0) {
        phi_f16 = temp_f16 + 4294967296.0f;
    }
    *(&D_800E2790 + (D_8004A7C4->unk0 * 4)) = phi_f16;
    func_800AF8C0_ovl1(temp_v0->unk8, 0xA, 6, *temp_a3);
    func_800A5B14_ovl1(arg0, 0xD0, 0xB0, 0x80, 0xFF);
loop_5:
    temp_v0_2 = *(&D_800E0D50 + (D_8004A7C4->unk0 * 4)) * 4;
    phi_v0 = temp_v0_2;
    if (*(&D_800E9C60 + temp_v0_2) != 0) {
        func_800ACBDC_ovl1(arg0);
        func_800B1900_ovl1(D_8004A7C4->unk2);
        phi_v0 = *(&D_800E0D50 + (D_8004A7C4->unk0 * 4)) * 4;
    }
    phi_a3 = 0;
    if (*(&D_800E9AA0 + phi_v0) != 0) {
        phi_a3 = 1;
    }
    func_800AF920_ovl1(phi_a3, phi_a3);
    func_8000B6BC_ovl1(1);
    goto loop_5;
}
#else
GLOBAL_ASM("asm/non_matchings/ovl1_13/ovl1_13/func_800BC328.s")
#endif

#ifdef MIPS_TO_C
//generated by mips_to_c commit e0e006e8858ba357d1dcb4dc64f038b7df278aa6
void func_800BC4C0(s32 arg0) {
    f32 temp_f16;
    f32 temp_f6;
    s32 temp_t0;
    s32 temp_v0_2;
    void *temp_a3;
    void *temp_v0;
    f32 phi_f6;
    f32 phi_f16;
    s32 phi_v0;
    ? phi_a3;

    temp_t0 = D_8004A7C4->unk0 * 4;
    temp_a3 = &D_800E9E20 + temp_t0;
    temp_v0 = (*temp_a3 * 4) + &D_800D5550;
    temp_f6 = (f32) temp_v0->unk0;
    phi_f6 = temp_f6;
    if (temp_v0->unk0 < 0) {
        phi_f6 = temp_f6 + 4294967296.0f;
    }
    *(&D_800E25D0 + temp_t0) = phi_f6;
    temp_f16 = (f32) temp_v0->unk4;
    phi_f16 = temp_f16;
    if (temp_v0->unk4 < 0) {
        phi_f16 = temp_f16 + 4294967296.0f;
    }
    *(&D_800E2790 + (D_8004A7C4->unk0 * 4)) = phi_f16;
    func_800AF8C0_ovl1(temp_v0->unk8, 0xA, 4, *temp_a3);
loop_5:
    temp_v0_2 = *(&D_800E0D50 + (D_8004A7C4->unk0 * 4)) * 4;
    phi_v0 = temp_v0_2;
    if (*(&D_800E9C60 + temp_v0_2) != 0) {
        func_800ACBDC_ovl1(arg0);
        func_800B1900_ovl1(D_8004A7C4->unk2);
        phi_v0 = *(&D_800E0D50 + (D_8004A7C4->unk0 * 4)) * 4;
    }
    phi_a3 = 0;
    if (*(&D_800E98E0 + phi_v0) == 0) {
        phi_a3 = 0;
        if (*(&D_800E9AA0 + phi_v0) != 0) {
            phi_a3 = 1;
        }
    }
    func_800AF920_ovl1(phi_a3, phi_a3);
    func_8000B6BC_ovl1(1);
    goto loop_5;
}
#else
GLOBAL_ASM("asm/non_matchings/ovl1_13/ovl1_13/func_800BC4C0.s")
#endif

#ifdef MIPS_TO_C
//generated by mips_to_c commit e0e006e8858ba357d1dcb4dc64f038b7df278aa6
void func_800BC664(s32 arg0) {
    f32 temp_f16;
    f32 temp_f6;
    s32 temp_t0;
    s32 temp_v0_2;
    void *temp_a3;
    void *temp_v0;
    f32 phi_f6;
    f32 phi_f16;
    s32 phi_v0;
    ? phi_a3;

    temp_t0 = D_8004A7C4->unk0 * 4;
    temp_a3 = &D_800E9E20 + temp_t0;
    temp_v0 = (*temp_a3 * 4) + &D_800D5574;
    temp_f6 = (f32) temp_v0->unk0;
    phi_f6 = temp_f6;
    if (temp_v0->unk0 < 0) {
        phi_f6 = temp_f6 + 4294967296.0f;
    }
    *(&D_800E25D0 + temp_t0) = phi_f6;
    temp_f16 = (f32) temp_v0->unk4;
    phi_f16 = temp_f16;
    if (temp_v0->unk4 < 0) {
        phi_f16 = temp_f16 + 4294967296.0f;
    }
    *(&D_800E2790 + (D_8004A7C4->unk0 * 4)) = phi_f16;
    func_800AF8C0_ovl1(temp_v0->unk8, 0xA, 6, *temp_a3);
    func_800A5B14_ovl1(arg0, 0xD0, 0xB0, 0x80, 0xFF);
loop_5:
    temp_v0_2 = *(&D_800E0D50 + (D_8004A7C4->unk0 * 4)) * 4;
    phi_v0 = temp_v0_2;
    if (*(&D_800E9C60 + temp_v0_2) != 0) {
        func_800ACBDC_ovl1(arg0);
        func_800B1900_ovl1(D_8004A7C4->unk2);
        phi_v0 = *(&D_800E0D50 + (D_8004A7C4->unk0 * 4)) * 4;
    }
    phi_a3 = 0;
    if (*(&D_800E9AA0 + phi_v0) != 0) {
        phi_a3 = 1;
    }
    func_800AF920_ovl1(phi_a3, phi_a3);
    func_8000B6BC_ovl1(1);
    goto loop_5;
}
#else
GLOBAL_ASM("asm/non_matchings/ovl1_13/ovl1_13/func_800BC664.s")
#endif

#ifdef MIPS_TO_C
//generated by mips_to_c commit e0e006e8858ba357d1dcb4dc64f038b7df278aa6
void func_800BC800(s32 arg0) {
    f32 temp_f16;
    f32 temp_f6;
    s32 temp_t0;
    s32 temp_v0_2;
    void *temp_a3;
    void *temp_v0;
    f32 phi_f6;
    f32 phi_f16;
    s32 phi_v0;
    ? phi_a3;

    temp_t0 = D_8004A7C4->unk0 * 4;
    temp_a3 = &D_800E9E20 + temp_t0;
    temp_v0 = (*temp_a3 * 4) + &D_800D5598;
    temp_f6 = (f32) temp_v0->unk0;
    phi_f6 = temp_f6;
    if (temp_v0->unk0 < 0) {
        phi_f6 = temp_f6 + 4294967296.0f;
    }
    *(&D_800E25D0 + temp_t0) = phi_f6;
    temp_f16 = (f32) temp_v0->unk4;
    phi_f16 = temp_f16;
    if (temp_v0->unk4 < 0) {
        phi_f16 = temp_f16 + 4294967296.0f;
    }
    *(&D_800E2790 + (D_8004A7C4->unk0 * 4)) = phi_f16;
    func_800AF8C0_ovl1(temp_v0->unk8, 0xA, 4, *temp_a3);
loop_5:
    temp_v0_2 = *(&D_800E0D50 + (D_8004A7C4->unk0 * 4)) * 4;
    phi_v0 = temp_v0_2;
    if (*(&D_800E9C60 + temp_v0_2) != 0) {
        func_800ACBDC_ovl1(arg0);
        func_800B1900_ovl1(D_8004A7C4->unk2);
        phi_v0 = *(&D_800E0D50 + (D_8004A7C4->unk0 * 4)) * 4;
    }
    phi_a3 = 0;
    if (*(&D_800E98E0 + phi_v0) != 0) {
        phi_a3 = 0;
        if (*(&D_800E9AA0 + phi_v0) != 0) {
            phi_a3 = 1;
        }
    }
    func_800AF920_ovl1(phi_a3, phi_a3);
    func_8000B6BC_ovl1(1);
    goto loop_5;
}
#else
GLOBAL_ASM("asm/non_matchings/ovl1_13/ovl1_13/func_800BC800.s")
#endif

#ifdef MIPS_TO_C
//generated by mips_to_c commit e0e006e8858ba357d1dcb4dc64f038b7df278aa6
void *func_800BC9A4(void *arg0) {
    void *temp_a0;
    void *temp_a1;
    void *temp_a2;
    void *temp_a3;
    void *temp_v0;
    void *temp_v1;
    void *phi_return;

    phi_return = gDisplayListHeads;
    if (*(&D_800E9AA0 + (*arg0 * 4)) != 0) {
        temp_a0 = gDisplayListHeads + 8;
        gDisplayListHeads->unk0 = 0xE7000000;
        gDisplayListHeads->unk4 = 0;
        temp_a1 = temp_a0 + 8;
        temp_a0->unk0 = 0xE3000A01;
        temp_a0->unk4 = 0;
        temp_a2 = temp_a1 + 8;
        temp_a1->unk0 = 0xE200001C;
        temp_a1->unk4 = 0xC084000;
        temp_a3 = temp_a2 + 8;
        temp_a2->unk0 = 0xFA000000;
        temp_a2->unk4 = 0xF4D8A2FF;
        temp_v1 = temp_a3 + 8;
        temp_a3->unk0 = 0xFCFFFFFF;
        temp_a3->unk4 = 0xFFFDF6FB;
        temp_v1->unk0 = 0xF65002D8;
        temp_v1->unk4 = 0;
        temp_v0 = temp_v1 + 8;
        gDisplayListHeads = temp_v0;
        phi_return = temp_v0;
    }
    return phi_return;
}
#else
GLOBAL_ASM("asm/non_matchings/ovl1_13/ovl1_13/func_800BC9A4.s")
#endif

#ifdef MIPS_TO_C
//generated by mips_to_c commit e0e006e8858ba357d1dcb4dc64f038b7df278aa6
void func_800BCA5C(void) {
    s32 sp3C;
    s32 temp_a3;
    s32 temp_v0;
    s32 temp_v1;
    void *temp_t9;
    s32 phi_a3;
    u16 phi_v0;

    temp_t9 = &D_800EC2E0 + (D_8004A7C4->unk0 * 4);
    if (*temp_t9 != 0) {
        *(&D_800D55BC + (*temp_t9 * 4))();
    }
    if (D_800BE4F0 == 0x21) {
        *(&D_800E9E20 + (D_8004A7C4->unk0 * 4)) = 2;
    } else {
        if (func_800F8560_ovl1() == 9) {
            *(&D_800E9E20 + (D_8004A7C4->unk0 * 4)) = 1;
        } else {
            *(&D_800E9E20 + (D_8004A7C4->unk0 * 4)) = 0;
        }
    }
    phi_a3 = 1;
loop_8:
    sp3C = phi_a3;
    temp_a3 = phi_a3;
    temp_v1 = func_800AEC70_ovl1(0x27, 0x3C, 0x50, phi_a3) * 4;
    *(&D_800EC2E0 + temp_v1) = temp_a3;
    temp_a3 = temp_a3 + 1;
    *(&D_800E9E20 + temp_v1) = (s32) (*(&D_800E9E20 + (D_8004A7C4->unk0 * 4)) * 3);
    phi_a3 = temp_a3;
    if (temp_a3 != 5) {
        goto loop_8;
    }
    func_800AF9B8_ovl1(0x28, 0xE, temp_a3);
    *(&D_800E98E0 + (D_8004A7C4->unk0 * 4)) = 0;
    *(&D_800E9C60 + (D_8004A7C4->unk0 * 4)) = 0;
    *(&D_800E9AA0 + (D_8004A7C4->unk0 * 4)) = 0;
    func_800A56F4_ovl1(0xA, 0xA, 0x136, 0xB6, 0xF0, 0xD8, 0xA0);
    func_800A5A14_ovl1(0, 0x10, 0);
    func_80020A20_ovl1(0, 0x5000, 0x10);
    func_80023884_ovl1();
    func_800A7678(0xED);
    if (D_800D6B24 != 0) {
loop_10:
        func_8000B6BC_ovl1(1);
        if (D_800D6B24 != 0) {
            goto loop_10;
        }
    }
    *(&D_800E9AA0 + (D_8004A7C4->unk0 * 4)) = 1;
    func_800A5A14_ovl1(0xFF, -0x10, 0);
    if (D_800D6B24 != 0) {
loop_12:
        func_8000B6BC_ovl1(1);
        if (D_800D6B24 != 0) {
            goto loop_12;
        }
    }
    func_8000B6BC_ovl1((s32) (3.0f * D_800D6B14));
loop_14:
    if ((D_80048F22 & 0x9000) == 0) {
        phi_v0 = D_80048F22;
        if ((D_80048F22 & 0x800) != 0) {
            func_800A7678(0x113);
            *(&D_800E98E0 + (D_8004A7C4->unk0 * 4)) = 0;
            phi_v0 = D_80048F22;
        }
        if ((phi_v0 & 0x400) != 0) {
            func_800A7678(0x113);
            *(&D_800E98E0 + (D_8004A7C4->unk0 * 4)) = 1;
        }
        func_8000B6BC_ovl1(1);
        goto loop_14;
    }
    func_800A7678(0xED);
    temp_v0 = D_8004A7C4->unk0 * 4;
    if ((1 == *(&D_800E98E0 + temp_v0)) && (1 != *(&D_800E9E20 + temp_v0))) {
        if (D_800BE4F0 == 0x21) {
            D_800D6B6C = 1;
        }
        D_800BE4F8 = 0;
        func_800A5744_ovl1(0, 0, 0);
        func_800A5A14_ovl1(0, 0x20, 2);
        func_80020A20_ovl1(0, 0, 8);
        if (D_800D6B24 != 0) {
loop_26:
            func_8000B6BC_ovl1(1);
            if (D_800D6B24 != 0) {
                goto loop_26;
            }
        }
        func_80020968_ovl1(0);
    } else {
        func_800A5A14_ovl1(0, 0x10, 0);
        if (D_800D6B24 != 0) {
loop_29:
            func_8000B6BC_ovl1(1);
            if (D_800D6B24 != 0) {
                goto loop_29;
            }
        }
        *(&D_800E9AA0 + (D_8004A7C4->unk0 * 4)) = 0;
        *(&D_800E9C60 + (D_8004A7C4->unk0 * 4)) = 1;
        func_80020A20_ovl1(0, 0x7800, 0x10);
        func_80023794_ovl1();
        func_800A5A14_ovl1(0xFF, -0x10, 0);
        if (D_800D6B24 != 0) {
loop_31:
            func_8000B6BC_ovl1(1);
            if (D_800D6B24 != 0) {
                goto loop_31;
            }
        }
        D_800BE544 = 0x8000;
    }
    func_800B1900_ovl1(D_8004A7C4->unk2);
}
#else
GLOBAL_ASM("asm/non_matchings/ovl1_13/ovl1_13/func_800BCA5C.s")
#endif

#ifdef MIPS_TO_C
//generated by mips_to_c commit e0e006e8858ba357d1dcb4dc64f038b7df278aa6
void func_800BCEEC(s32 arg0) {
    s32 temp_v1;
    void *temp_t6;
    void *temp_v0;

    temp_v0 = func_800AC954_ovl1(0, &D_800ED500);
    temp_v0->unk13 = (u8) (temp_v0->unk13 | 4);
    func_8000A5FC_ovl1(arg0, &D_800AD1A0, 0xA, 0x80000000, 0xA);
    func_800AF96C_ovl1(1);
    *(&D_800E25D0 + (*D_8004A7C4 * 4)) = 7.0f;
    *(&D_800E2790 + (*D_8004A7C4 * 4)) = 182.0f;
    *(&D_800E9FE0 + (*D_8004A7C4 * 4)) = 0;
    temp_v1 = *D_8004A7C4 * 4;
    temp_t6 = &D_800E9FE0 + temp_v1;
    *(&D_800E9AA0 + temp_v1) = (s32) *temp_t6;
    *(&D_800E98E0 + (*D_8004A7C4 * 4)) = (s32) *temp_t6;
    func_800AFA14_ovl1(*temp_t6, &D_800E9FE0);
}
#else
GLOBAL_ASM("asm/non_matchings/ovl1_13/ovl1_13/func_800BCEEC.s")
#endif

#ifdef MIPS_TO_C
//generated by mips_to_c commit e0e006e8858ba357d1dcb4dc64f038b7df278aa6
s32 func_800BCFD8(s32 arg0, s32 arg1) {
    s32 temp_a1;
    void *temp_a0;
    void *temp_v1;
    void *phi_v1;
    void *phi_a0;
    s32 phi_a1;

    phi_v1 = func_800A8BAC_ovl1(*(&D_800D5460 + ((D_800D6BB0 * 0x28) + (arg1 * 4)))) + 0x10;
    phi_a0 = ((arg0 * 2) + 0x1400) + &D_800ED510;
    phi_a1 = 0;
loop_1:
    temp_a1 = phi_a1 + 2;
    temp_a0 = phi_a0 + 0x500;
    temp_a0->unk-500 = (s32) phi_v1->unk0;
    temp_v1 = phi_v1 + 0x50;
    temp_a0->unk-4FC = (s32) phi_v1->unk4;
    temp_a0->unk-4F8 = (s32) temp_v1->unk-48;
    temp_a0->unk-4F4 = (s32) temp_v1->unk-44;
    temp_a0->unk-4F0 = (s32) temp_v1->unk-40;
    temp_a0->unk-4EC = (s32) temp_v1->unk-3C;
    temp_a0->unk-4E8 = (s32) temp_v1->unk-38;
    temp_a0->unk-4E4 = (s32) temp_v1->unk-34;
    temp_a0->unk-4E0 = (s32) temp_v1->unk-30;
    temp_a0->unk-280 = (s32) temp_v1->unk-28;
    temp_a0->unk-27C = (s32) temp_v1->unk-24;
    temp_a0->unk-278 = (s32) temp_v1->unk-20;
    temp_a0->unk-274 = (s32) temp_v1->unk-1C;
    temp_a0->unk-270 = (s32) temp_v1->unk-18;
    temp_a0->unk-26C = (s32) temp_v1->unk-14;
    temp_a0->unk-268 = (s32) temp_v1->unk-10;
    temp_a0->unk-264 = (s32) temp_v1->unk-C;
    temp_a0->unk-260 = (s32) temp_v1->unk-8;
    phi_v1 = temp_v1;
    phi_a0 = temp_a0;
    phi_a1 = temp_a1;
    if (temp_a1 != 0x1E) {
        goto loop_1;
    }
    return 0x1E;
}
#else
GLOBAL_ASM("asm/non_matchings/ovl1_13/ovl1_13/func_800BCFD8.s")
#endif

#ifdef MIPS_TO_C
//generated by mips_to_c commit e0e006e8858ba357d1dcb4dc64f038b7df278aa6
s32 func_800BD0E8(s32 arg0, s32 arg1) {
    s32 temp_a1;
    void *temp_a0;
    void *temp_v1;
    void *phi_v1;
    void *phi_a0;
    s32 phi_a1;

    phi_v1 = func_800A8BAC_ovl1(*(&D_800D53DC + ((D_800D6BB0 * 8) + (arg1 * 4)))) + 0x10;
    phi_a0 = ((arg0 * 0x28) + 0xAB4) + &D_800ED510;
    phi_a1 = 0;
loop_1:
    temp_a1 = phi_a1 + 2;
    temp_v1 = phi_v1 + 0x50;
    *phi_a0 = (s32) *phi_v1;
    temp_a0 = phi_a0 + 0x500;
    temp_a0->unk-4FC = (s32) temp_v1->unk-4C;
    temp_a0->unk-4F8 = (s32) temp_v1->unk-48;
    temp_a0->unk-4F4 = (s32) temp_v1->unk-44;
    temp_a0->unk-4F0 = (s32) temp_v1->unk-40;
    temp_a0->unk-4EC = (s32) temp_v1->unk-3C;
    temp_a0->unk-4E8 = (s32) temp_v1->unk-38;
    temp_a0->unk-4E4 = (s32) temp_v1->unk-34;
    temp_a0->unk-4E0 = (s32) temp_v1->unk-30;
    temp_a0->unk-4DC = (s32) temp_v1->unk-2C;
    temp_a0->unk-280 = (s32) temp_v1->unk-28;
    temp_a0->unk-27C = (s32) temp_v1->unk-24;
    temp_a0->unk-278 = (s32) temp_v1->unk-20;
    temp_a0->unk-274 = (s32) temp_v1->unk-1C;
    temp_a0->unk-270 = (s32) temp_v1->unk-18;
    temp_a0->unk-26C = (s32) temp_v1->unk-14;
    temp_a0->unk-268 = (s32) temp_v1->unk-10;
    temp_a0->unk-264 = (s32) temp_v1->unk-C;
    temp_a0->unk-260 = (s32) temp_v1->unk-8;
    temp_a0->unk-25C = (s32) temp_v1->unk-4;
    phi_v1 = temp_v1;
    phi_a0 = temp_a0;
    phi_a1 = temp_a1;
    if (temp_a1 != 0x1A) {
        goto loop_1;
    }
    return 0x1A;
}
#else
GLOBAL_ASM("asm/non_matchings/ovl1_13/ovl1_13/func_800BD0E8.s")
#endif

#ifdef MIPS_TO_C
//generated by mips_to_c commit e0e006e8858ba357d1dcb4dc64f038b7df278aa6
s32 func_800BD208(s32 arg0, s32 arg1) {
    s32 temp_t7;
    s32 temp_v0;
    void *temp_a0;
    void *temp_a0_2;
    void *temp_v1;
    void *temp_v1_2;
    void *phi_v1;
    void *phi_a0;
    s32 phi_v0;

    temp_v1 = func_800A8BAC_ovl1(*(&D_800D5408 + ((D_800D6BB0 * 8) + (arg1 * 4))));
    temp_a0 = ((arg0 * 8) + 0x4E34) + &D_800ED510;
    *temp_a0 = (s32) temp_v1->unk10;
    temp_t7 = temp_v1->unk14;
    temp_a0 = temp_a0 + 0x780;
    temp_v1 = temp_v1 + 0x28;
    temp_a0->unk-77C = temp_t7;
    temp_a0->unk-500 = (s32) temp_v1->unk-10;
    temp_a0->unk-4FC = (s32) temp_v1->unk-C;
    temp_a0->unk-280 = (s32) temp_v1->unk-8;
    temp_a0->unk-27C = (s32) temp_v1->unk-4;
    phi_v1 = temp_v1;
    phi_a0 = temp_a0;
    phi_v0 = 3;
loop_1:
    temp_v0 = phi_v0 + 4;
    temp_v1_2 = phi_v1 + 0x20;
    *phi_a0 = (s32) *phi_v1;
    temp_a0_2 = phi_a0 + 0xA00;
    temp_a0_2->unk-9FC = (s32) temp_v1_2->unk-1C;
    temp_a0_2->unk-780 = (s32) temp_v1_2->unk-18;
    temp_a0_2->unk-77C = (s32) temp_v1_2->unk-14;
    temp_a0_2->unk-500 = (s32) temp_v1_2->unk-10;
    temp_a0_2->unk-4FC = (s32) temp_v1_2->unk-C;
    temp_a0_2->unk-280 = (s32) temp_v1_2->unk-8;
    temp_a0_2->unk-27C = (s32) temp_v1_2->unk-4;
    phi_v1 = temp_v1_2;
    phi_a0 = temp_a0_2;
    phi_v0 = temp_v0;
    if (temp_v0 != 0xB) {
        goto loop_1;
    }
    return temp_v0;
}
#else
GLOBAL_ASM("asm/non_matchings/ovl1_13/ovl1_13/func_800BD208.s")
#endif

#ifdef MIPS_TO_C
//generated by mips_to_c commit e0e006e8858ba357d1dcb4dc64f038b7df278aa6
void func_800BD2F4(s32 arg0) {
    s32 temp_s0;
    s32 phi_s1;
    s32 phi_s0;
    s32 phi_s1_2;

    phi_s1 = arg0;
    if (arg0 < 0) {
        phi_s1 = 0;
    }
    phi_s1_2 = phi_s1;
    if (phi_s1 >= 0x1F) {
        phi_s1_2 = 0x1E;
    }
    phi_s0 = 0;
loop_5:
    if (phi_s0 < phi_s1_2) {
        func_800BD374_ovl1(phi_s0, 1);
    } else {
        func_800BD374_ovl1(phi_s0, 0);
    }
    temp_s0 = phi_s0 + 1;
    phi_s0 = temp_s0;
    if (temp_s0 != 0x1E) {
        goto loop_5;
    }
}
#else
GLOBAL_ASM("asm/non_matchings/ovl1_13/ovl1_13/func_800BD2F4.s")
#endif

#ifdef MIPS_TO_C
//generated by mips_to_c commit e0e006e8858ba357d1dcb4dc64f038b7df278aa6
s32 func_800BD374(s32 arg0, s32 arg1) {
    s32 temp_t7;
    s32 temp_v0;
    void *temp_a0;
    void *temp_a0_2;
    void *temp_v1;
    void *temp_v1_2;
    void *phi_v1;
    void *phi_a0;
    s32 phi_v0;

    temp_v1 = func_800A8BAC_ovl1(*(&D_800D5434 + ((D_800D6BB0 * 8) + (arg1 * 4))));
    temp_a0 = ((arg0 * 8) + 0x4E34) + &D_800ED510;
    *temp_a0 = (s32) temp_v1->unk10;
    temp_t7 = temp_v1->unk14;
    temp_a0 = temp_a0 + 0x780;
    temp_v1 = temp_v1 + 0x28;
    temp_a0->unk-77C = temp_t7;
    temp_a0->unk-500 = (s32) temp_v1->unk-10;
    temp_a0->unk-4FC = (s32) temp_v1->unk-C;
    temp_a0->unk-280 = (s32) temp_v1->unk-8;
    temp_a0->unk-27C = (s32) temp_v1->unk-4;
    phi_v1 = temp_v1;
    phi_a0 = temp_a0;
    phi_v0 = 3;
loop_1:
    temp_v0 = phi_v0 + 4;
    temp_v1_2 = phi_v1 + 0x20;
    *phi_a0 = (s32) *phi_v1;
    temp_a0_2 = phi_a0 + 0xA00;
    temp_a0_2->unk-9FC = (s32) temp_v1_2->unk-1C;
    temp_a0_2->unk-780 = (s32) temp_v1_2->unk-18;
    temp_a0_2->unk-77C = (s32) temp_v1_2->unk-14;
    temp_a0_2->unk-500 = (s32) temp_v1_2->unk-10;
    temp_a0_2->unk-4FC = (s32) temp_v1_2->unk-C;
    temp_a0_2->unk-280 = (s32) temp_v1_2->unk-8;
    temp_a0_2->unk-27C = (s32) temp_v1_2->unk-4;
    phi_v1 = temp_v1_2;
    phi_a0 = temp_a0_2;
    phi_v0 = temp_v0;
    if (temp_v0 != 0xB) {
        goto loop_1;
    }
    return temp_v0;
}
#else
GLOBAL_ASM("asm/non_matchings/ovl1_13/ovl1_13/func_800BD374.s")
#endif

#ifdef MIPS_TO_C
//generated by mips_to_c commit e0e006e8858ba357d1dcb4dc64f038b7df278aa6
s32 func_800BD460(s32 arg0, s32 arg1) {
    s32 temp_a1;
    void *temp_a0;
    void *temp_v1;
    void *phi_v1;
    void *phi_a0;
    s32 phi_a1;

    phi_v1 = func_800A8BAC_ovl1(*(&D_800D5310 + ((D_800D6BB0 * 0x28) + (arg1 * 4)))) + 0x10;
    phi_a0 = ((arg0 * 2) + 0x1180) + &D_800ED510;
    phi_a1 = 0;
loop_1:
    temp_a1 = phi_a1 + 4;
    phi_a0->unk0 = (s32) phi_v1->unk0;
    phi_a0->unk4 = (s32) phi_v1->unk4;
    temp_v1 = phi_v1 + 0xC0;
    phi_a0->unkC = (s32) phi_v1->unkC;
    phi_a0->unk8 = (s32) phi_v1->unk8;
    temp_a0 = phi_a0 + 0xA00;
    temp_a0->unk-9EC = (s32) temp_v1->unk-AC;
    temp_a0->unk-9F0 = (s32) temp_v1->unk-B0;
    temp_a0->unk-9E4 = (s32) temp_v1->unk-A4;
    temp_a0->unk-9E8 = (s32) temp_v1->unk-A8;
    temp_a0->unk-9DC = (s32) temp_v1->unk-9C;
    temp_a0->unk-9E0 = (s32) temp_v1->unk-A0;
    temp_a0->unk-9D4 = (s32) temp_v1->unk-94;
    temp_a0->unk-9D8 = (s32) temp_v1->unk-98;
    temp_a0->unk-77C = (s32) temp_v1->unk-8C;
    temp_a0->unk-780 = (s32) temp_v1->unk-90;
    temp_a0->unk-774 = (s32) temp_v1->unk-84;
    temp_a0->unk-778 = (s32) temp_v1->unk-88;
    temp_a0->unk-76C = (s32) temp_v1->unk-7C;
    temp_a0->unk-770 = (s32) temp_v1->unk-80;
    temp_a0->unk-764 = (s32) temp_v1->unk-74;
    temp_a0->unk-768 = (s32) temp_v1->unk-78;
    temp_a0->unk-75C = (s32) temp_v1->unk-6C;
    temp_a0->unk-760 = (s32) temp_v1->unk-70;
    temp_a0->unk-754 = (s32) temp_v1->unk-64;
    temp_a0->unk-758 = (s32) temp_v1->unk-68;
    temp_a0->unk-4FC = (s32) temp_v1->unk-5C;
    temp_a0->unk-500 = (s32) temp_v1->unk-60;
    temp_a0->unk-4F4 = (s32) temp_v1->unk-54;
    temp_a0->unk-4F8 = (s32) temp_v1->unk-58;
    temp_a0->unk-4EC = (s32) temp_v1->unk-4C;
    temp_a0->unk-4F0 = (s32) temp_v1->unk-50;
    temp_a0->unk-4E4 = (s32) temp_v1->unk-44;
    temp_a0->unk-4E8 = (s32) temp_v1->unk-48;
    temp_a0->unk-4DC = (s32) temp_v1->unk-3C;
    temp_a0->unk-4E0 = (s32) temp_v1->unk-40;
    temp_a0->unk-4D4 = (s32) temp_v1->unk-34;
    temp_a0->unk-4D8 = (s32) temp_v1->unk-38;
    temp_a0->unk-27C = (s32) temp_v1->unk-2C;
    temp_a0->unk-280 = (s32) temp_v1->unk-30;
    temp_a0->unk-274 = (s32) temp_v1->unk-24;
    temp_a0->unk-278 = (s32) temp_v1->unk-28;
    temp_a0->unk-26C = (s32) temp_v1->unk-1C;
    temp_a0->unk-270 = (s32) temp_v1->unk-20;
    temp_a0->unk-264 = (s32) temp_v1->unk-14;
    temp_a0->unk-268 = (s32) temp_v1->unk-18;
    temp_a0->unk-25C = (s32) temp_v1->unk-C;
    temp_a0->unk-260 = (s32) temp_v1->unk-10;
    temp_a0->unk-254 = (s32) temp_v1->unk-4;
    temp_a0->unk-258 = (s32) temp_v1->unk-8;
    phi_v1 = temp_v1;
    phi_a0 = temp_a0;
    phi_a1 = temp_a1;
    if (temp_a1 != 0x20) {
        goto loop_1;
    }
    return 0x20;
}
#else
GLOBAL_ASM("asm/non_matchings/ovl1_13/ovl1_13/func_800BD460.s")
#endif

#ifdef MIPS_TO_C
//generated by mips_to_c commit e0e006e8858ba357d1dcb4dc64f038b7df278aa6
s32 func_800BD660(void) {
    s32 sp18;
    s32 temp_a2;
    s32 temp_a2_2;
    s32 phi_a2;
    s32 phi_a2_2;
    s32 phi_return;

    temp_a2 = D_800D6E88 - 1;
    phi_return = D_800D6E88;
    if (D_800D6E88 != D_800D6EA0) {
        D_800D6EA0 = (s32) D_800D6E88;
        phi_a2 = temp_a2;
        if (temp_a2 >= 0x64) {
            phi_a2 = 0x63;
        }
        phi_a2_2 = phi_a2;
        if (phi_a2 < 0) {
            phi_a2_2 = 0;
        }
        sp18 = phi_a2_2;
        func_800BCFD8_ovl1(0x22, phi_a2_2 / 0xA, phi_a2_2);
        temp_a2_2 = phi_a2_2;
        phi_return = func_800BCFD8_ovl1(0x34, temp_a2_2 % 0xA, temp_a2_2);
    }
    return phi_return;
}
#else
GLOBAL_ASM("asm/non_matchings/ovl1_13/ovl1_13/func_800BD660.s")
#endif

#ifdef MIPS_TO_C
//generated by mips_to_c commit e0e006e8858ba357d1dcb4dc64f038b7df278aa6
void func_800BD6E0(void) {
    s32 temp_a0;
    s32 temp_a0_2;
    s32 temp_at;
    s32 temp_t4;
    s32 temp_v0;
    s32 phi_v0;
    s32 phi_a0;
    ? phi_a1;

    if (1 == D_800D6E8C) {
        if (D_800D6EC0 == 0) {
            if (func_800F8560_ovl1(1) != 3) {
                if (D_800D6B24 == 0) {
                    func_800A7678(0xEC);
                    D_800D6EC0 = (s32) (D_800D6EC0 + 1);
                }
            }
        }
    }
    if (D_800D6EA4 == D_800D6E8C) {
        D_800D6EB4 = 0;
        return;
    }
    temp_v0 = D_800D6EB4 + 1;
    D_800D6EB4 = temp_v0;
    phi_v0 = temp_v0;
    if (temp_v0 >= 5) {
        D_800D6EB4 = 1;
        phi_v0 = 1;
    }
    if (1 == phi_v0) {
        if (D_800D6E8C < D_800D6EA4) {
            temp_a0 = D_800D6EA4 - 1;
            D_800D6EA4 = temp_a0;
            phi_a0 = temp_a0;
            phi_a1 = 0;
        } else {
            func_800A7678(0xD3, 0, &D_800D6EA4, 1);
            temp_t4 = D_800D6EA4 + 1;
            temp_at = temp_t4 < 2;
            D_800D6EA4 = temp_t4;
            temp_a0_2 = temp_t4 - 1;
            phi_a0 = temp_a0_2;
            phi_a1 = 1;
            if (temp_at == 0) {
                D_800D6EC0 = 0;
                phi_a0 = temp_a0_2;
                phi_a1 = 1;
            }
        }
        func_800BD0E8_ovl1(phi_a0, phi_a1, &D_800D6EA4);
    }
}
#else
GLOBAL_ASM("asm/non_matchings/ovl1_13/ovl1_13/func_800BD6E0.s")
#endif

#ifdef MIPS_TO_C
//generated by mips_to_c commit e0e006e8858ba357d1dcb4dc64f038b7df278aa6
void func_800BD7FC(void) {
    s32 temp_s0;
    s32 temp_t1;
    s32 temp_t7;
    s32 temp_v0;
    void *temp_t0;
    s32 phi_s0;
    ? phi_s1;

    temp_t7 = D_800D6EBC - 1;
    D_800D6EBC = temp_t7;
    if (temp_t7 <= 0) {
        D_800D6EBC = 0;
        if (D_800F4D10 != 0) {
loop_2:
            temp_v0 = D_800F4D10 + 1;
            temp_t0 = &D_800D55D0 + (temp_v0 * 4);
            D_800F4D10 = temp_v0;
            D_800D6EBC = (s32) *temp_t0;
            if (*temp_t0 >= 0) {
                phi_s1 = 0;
                if (*temp_t0 >= 0xA) {
                    phi_s1 = 1;
                }
                phi_s0 = 0;
loop_7:
                func_800BD374_ovl1(phi_s0, phi_s1);
                temp_s0 = phi_s0 + 1;
                phi_s0 = temp_s0;
                if (temp_s0 != 0x1E) {
                    goto loop_7;
                }
                return;
            }
            D_800F4D10 = 0;
            D_800D6EBC = 0;
            return;
        }
        temp_t1 = D_800D6E98 + 1;
        if (D_800D6E60 != D_800D6E98) {
            D_800D6E98 = temp_t1;
            if (temp_t1 >= 0x1E) {
                D_800F4D10 = 0;
                D_800D6E60 = (s32) (D_800D6E60 - 0x1E);
                D_800D6E98 = 0;
                func_800A7678(1, &D_800D6E60);
                func_800BC258_ovl1(1);
                goto loop_2;
            }
            D_800D6EBC = 3;
            func_800A7678(0x117, &D_800D6E60);
            func_800BD374_ovl1(D_800D6E98 - 1, 1);
        }
    }
}
#else
GLOBAL_ASM("asm/non_matchings/ovl1_13/ovl1_13/func_800BD7FC.s")
#endif

#ifdef MIPS_TO_C
//generated by mips_to_c commit e0e006e8858ba357d1dcb4dc64f038b7df278aa6
void func_800BD92C(void) {
    s32 temp_s0;
    s32 temp_s0_2;
    s32 temp_s0_3;
    s32 temp_t2;
    s32 temp_t6;
    s32 temp_v0;
    s32 phi_v0;
    s32 phi_s0;
    s32 phi_s0_2;
    ? phi_a1;

    if (0.0f == D_800D6E5C) {
        D_800D6EB8 = 0;
        return;
    }
    if (D_800D6EAC == D_800D6E94) {
        if (D_800D6E9C != 0) {
            D_800D6EB8 = 0;
            return;
        }
    }
    temp_v0 = D_800D6EB8 + 1;
    D_800D6EB8 = temp_v0;
    phi_v0 = temp_v0;
    if (temp_v0 >= 3) {
        D_800D6EB8 = 1;
        phi_v0 = 1;
    }
    if (phi_v0 == 1) {
        if (D_800D6E94 == 0) {
            phi_s0 = 0;
loop_10:
            func_800BD208_ovl1(phi_s0, 0);
            temp_s0 = phi_s0 + 1;
            phi_s0 = temp_s0;
            if (temp_s0 != 0x1E) {
                goto loop_10;
            }
            D_800D6E94 = 0;
            D_800D6EAC = 0;
            return;
        }
        if (D_800D6E9C == 0) {
            func_800A7678(0xD3, D_800D6E94, &D_800D6E9C);
            temp_t2 = D_800D6EAC + 1;
            D_800D6EAC = temp_t2;
            temp_s0_2 = temp_t2 - 1;
            phi_s0_2 = temp_s0_2;
            phi_a1 = 1;
            if (0x1E == temp_t2) {
                D_800D6E9C = (s32) (D_800D6E9C + 1);
                phi_s0_2 = temp_s0_2;
                phi_a1 = 1;
            }
        } else {
            if (D_800D6E94 < D_800D6EAC) {
                temp_s0_3 = D_800D6EAC - 1;
                D_800D6EAC = temp_s0_3;
                phi_s0_2 = temp_s0_3;
                phi_a1 = 0;
            } else {
                func_800A7678(0xD3, 0, &D_800D6E9C);
                temp_t6 = D_800D6EAC + 1;
                D_800D6EAC = temp_t6;
                phi_s0_2 = temp_t6 - 1;
                phi_a1 = 1;
            }
        }
        func_800BD208_ovl1(phi_s0_2, phi_a1);
    }
}
#else
GLOBAL_ASM("asm/non_matchings/ovl1_13/ovl1_13/func_800BD92C.s")
#endif

#ifdef MIPS_TO_C
//generated by mips_to_c commit e0e006e8858ba357d1dcb4dc64f038b7df278aa6
s32 func_800BDAA8(void) {
    s32 phi_v0;
    s32 phi_return;

    phi_v0 = D_800D6E90;
    phi_return = D_800D6E90;
    if (D_800D6EA8 != D_800D6E90) {
        func_800BD460_ovl1(0xE4, *(&D_800D55F8 + (D_800D6E90 * 2)));
        func_800BD460_ovl1(0x108, *(&D_800D55F9 + (D_800D6E90 * 2)));
        phi_v0 = D_800D6E90;
        phi_return = D_800D6E90;
    }
    D_800D6EA8 = phi_v0;
    return phi_return;
}
#else
GLOBAL_ASM("asm/non_matchings/ovl1_13/ovl1_13/func_800BDAA8.s")
#endif

#ifdef MIPS_TO_C
//generated by mips_to_c commit e0e006e8858ba357d1dcb4dc64f038b7df278aa6
void *func_800BDB18(void) {
    u32 temp_s0;
    u32 temp_v0;
    u32 temp_v0_2;
    u32 phi_s0;
    u32 phi_v0;
    s32 phi_s1;

    if (D_800D6E60 >= 0x1E) {
loop_1:
        D_800D6E60 = (s32) (D_800D6E60 - 0x1E);
        func_800BC258_ovl1(1);
        if (D_800D6E60 >= 0x1E) {
            goto loop_1;
        }
    }
    if ((D_800BE508 != 0) || (phi_s1 = D_800D6E60, (D_800BE4FC != 0))) {
        temp_v0 = (u32) gKirbyHp;
        phi_s0 = 0U;
        phi_v0 = temp_v0;
        phi_s1 = D_800D6E60;
        if (temp_v0 != 0) {
loop_5:
            func_800BD0E8_ovl1(phi_s0, 1);
            temp_s0 = phi_s0 + 1;
            temp_v0_2 = (u32) gKirbyHp;
            phi_s0 = temp_s0;
            if (temp_s0 < temp_v0_2) {
                goto loop_5;
            }
            phi_v0 = temp_v0_2;
            phi_s1 = D_800D6E60;
        }
        D_800D6EA4 = phi_v0;
        D_800D6E8C = phi_v0;
    } else {

    }
    func_800BD2F4_ovl1(phi_s1);
    D_800D6EB0 = (s32) D_800D6E60;
    D_800D6E98 = (s32) D_800D6E60;
    return &D_800D6EB0;
}
#else
GLOBAL_ASM("asm/non_matchings/ovl1_13/ovl1_13/func_800BDB18.s")
#endif

#ifdef MIPS_TO_C
//generated by mips_to_c commit e0e006e8858ba357d1dcb4dc64f038b7df278aa6
void func_800BDD08(void) {
    func_800BD660_ovl1();
    if (D_800D6B30 != 0) {
        func_800BD6E0_ovl1();
        if (D_800D6E94 == 0) {
            func_800BD7FC_ovl1();
        }
        func_800BD92C_ovl1();
    }
    func_800BDAA8_ovl1();
}
#else
GLOBAL_ASM("asm/non_matchings/ovl1_13/ovl1_13/func_800BDD08.s")
#endif

#ifdef MIPS_TO_C
//generated by mips_to_c commit e0e006e8858ba357d1dcb4dc64f038b7df278aa6
void func_800BDD68(void) {
    func_800BDAA8_ovl1();
}
#else
GLOBAL_ASM("asm/non_matchings/ovl1_13/ovl1_13/func_800BDD68.s")
#endif

#ifdef MIPS_TO_C
//generated by mips_to_c commit e0e006e8858ba357d1dcb4dc64f038b7df278aa6
void func_800BDD88(s32 arg0) {

}
#else
GLOBAL_ASM("asm/non_matchings/ovl1_13/ovl1_13/func_800BDD88.s")
#endif

#ifdef MIPS_TO_C
//generated by mips_to_c commit e0e006e8858ba357d1dcb4dc64f038b7df278aa6
void func_800BDD90(s32 arg0) {

}
#else
GLOBAL_ASM("asm/non_matchings/ovl1_13/ovl1_13/func_800BDD90.s")
#endif

#ifdef MIPS_TO_C
//generated by mips_to_c commit e0e006e8858ba357d1dcb4dc64f038b7df278aa6
void *func_800BDD98(void) {
    D_800D6EA8 = -1;
    D_800D6EA0 = 0;
    D_800D6EB0 = 0;
    D_800D6EAC = 0;
    D_800D6EA4 = 0;
    D_800D6EC0 = 0;
    D_800F4D10 = 0;
    D_800D6EBC = 0;
    D_800D6EB8 = 0;
    D_800D6EB4 = 0;
    return &D_800D6EA0;
}
#else
GLOBAL_ASM("asm/non_matchings/ovl1_13/ovl1_13/func_800BDD98.s")
#endif

#ifdef MIPS_TO_C
//generated by mips_to_c commit e0e006e8858ba357d1dcb4dc64f038b7df278aa6
void func_800BDE0C(s32 arg0) {
    s32 temp_v1;
    void *temp_a2;
    void *temp_v0;
    void *phi_a2;
    void *phi_v0;
    s32 phi_v1;
    void *phi_a3;

    D_800D6F58 = &D_800ED510;
    if (D_800F4D14 != 0) {
        if (D_800F6198 != 0) {
            func_800A8934_ovl1(0x50002, 0x10, 0, &D_800ED510);
            phi_a2 = &D_800EDA24;
            phi_a3 = &D_800EDA10;
loop_3:
            phi_a2->unk0 = (u16) D_800EDA60;
            phi_a2->unk2 = (u16) D_800EDA60;
            phi_v0 = phi_a3 + 0x18;
            phi_v1 = 0xC;
loop_4:
            temp_v1 = phi_v1 + 4;
            phi_v0->unk2 = (u16) D_800EDA60;
            phi_v0->unk4 = (u16) D_800EDA60;
            phi_v0->unk6 = (u16) D_800EDA60;
            temp_v0 = phi_v0 + 8;
            temp_v0->unk-8 = (u16) D_800EDA60;
            phi_v0 = temp_v0;
            phi_v1 = temp_v1;
            if (temp_v1 != 0xD8) {
                goto loop_4;
            }
            temp_a2 = phi_a2 + 0x280;
            phi_a2 = temp_a2;
            phi_a3 = phi_a3 + 0x280;
            if (temp_a2 != &D_800F4324) {
                goto loop_3;
            }
        } else {
            D_800D6F50 = 0;
            func_800A8934_ovl1(*(&D_800D52FC + (D_800D6BB0 * 4)), 0x10, 0, &D_800ED510);
            func_800BDB18_ovl1();
        }
        D_800F4D14 = 0;
    }
    if (D_800F6198 != 0) {
        func_800BDD68_ovl1();
        return;
    }
    func_800BDD08_ovl1();
}
#else
GLOBAL_ASM("asm/non_matchings/ovl1_13/ovl1_13/func_800BDE0C.s")
#endif

#ifdef MIPS_TO_C
//generated by mips_to_c commit e0e006e8858ba357d1dcb4dc64f038b7df278aa6
void func_800BDF2C(void) {
    s32 temp_a1;
    s32 temp_t7;
    void *temp_a0;
    void *phi_v1;
    void *phi_a0;
    s32 phi_a1;

    temp_t7 = D_800D6F50 << 8;
    phi_v1 = temp_t7 + &D_800F5770;
    phi_a0 = temp_t7 + &D_800F4D70;
    phi_a1 = 0;
loop_1:
    temp_a1 = phi_a1 + 4;
    phi_v1->unk0 = 0xFFFE7961;
    phi_a0->unk0 = 0;
    phi_v1->unk4 = 0xFFFE7961;
    phi_a0->unk4 = 0;
    phi_v1->unk8 = 0xFFFE7961;
    phi_a0->unk8 = 0;
    phi_v1->unkC = 0xFFFE7961;
    phi_a0->unkC = 0;
    temp_a0 = phi_a0 + 0x10;
    phi_v1 = phi_v1 + 0x10;
    phi_a0 = temp_a0;
    phi_a1 = temp_a1;
    if (temp_a1 != 0x40) {
        goto loop_1;
    }
    func_800BDD98_ovl1(temp_a0, temp_a1, 0x40);
    D_800F4D14 = 1;
}
#else
GLOBAL_ASM("asm/non_matchings/ovl1_13/ovl1_13/func_800BDF2C.s")
#endif

#ifdef MIPS_TO_C
//generated by mips_to_c commit e0e006e8858ba357d1dcb4dc64f038b7df278aa6
void func_800BDFB8(s32 arg0, s32 arg1, u32 arg2) {
    s32 temp_a0;
    u32 temp_s0;
    void *phi_s1;
    u32 phi_s0;

    if (arg2 != 0) {
        phi_s1 = arg0 + (arg1 * 4);
        phi_s0 = 0U;
loop_2:
        temp_a0 = *phi_s1;
        if (0xFFFF != temp_a0) {
            func_800A8BAC_ovl1(temp_a0);
            temp_s0 = phi_s0 + 1;
            phi_s1 = phi_s1 + 4;
            phi_s0 = temp_s0;
            if (temp_s0 < arg2) {
                goto loop_2;
            }
        }
    }
}
#else
GLOBAL_ASM("asm/non_matchings/ovl1_13/ovl1_13/func_800BDFB8.s")
#endif

#ifdef MIPS_TO_C
//generated by mips_to_c commit e0e006e8858ba357d1dcb4dc64f038b7df278aa6
void func_800BE028(s32 arg0, s32 arg1, u32 arg2) {
    s32 temp_a0;
    u32 temp_s0;
    void *phi_s1;
    u32 phi_s0;

    if (arg2 != 0) {
        phi_s1 = arg0 + (arg1 * 4);
        phi_s0 = 0U;
loop_2:
        temp_a0 = *phi_s1;
        if (0xFFFF != temp_a0) {
            func_800A8A7C_ovl1(temp_a0);
            temp_s0 = phi_s0 + 1;
            phi_s1 = phi_s1 + 4;
            phi_s0 = temp_s0;
            if (temp_s0 < arg2) {
                goto loop_2;
            }
        }
    }
}
#else
GLOBAL_ASM("asm/non_matchings/ovl1_13/ovl1_13/func_800BE028.s")
#endif

#ifdef MIPS_TO_C
//generated by mips_to_c commit e0e006e8858ba357d1dcb4dc64f038b7df278aa6
s32 func_800BE098(void) {
    s32 sp18;
    s32 temp_a1;
    s32 temp_v0;
    void *temp_a3;
    void *temp_at;
    void *phi_t0;
    void *phi_t1;
    void *phi_t2;
    void *phi_v0;
    void *phi_v1;
    s32 phi_a1;
    void *phi_a3;
    void *phi_t3;

    D_800F4D18 = 2;
    phi_t0 = &D_800F6170;
    phi_t1 = &D_800F4D48;
    phi_t2 = &D_800F4D20;
    phi_a3 = &D_800F4D70;
    phi_t3 = &D_800F5770;
loop_1:
    *phi_t0 = 0;
    *phi_t1 = 0;
    *phi_t2 = 0;
    phi_v0 = phi_t3;
    phi_v1 = phi_a3;
    phi_a1 = 0;
loop_2:
    temp_a1 = phi_a1 + 4;
    phi_v0->unk0 = 0xFFFE7961;
    phi_v1->unk0 = 0;
    phi_v0->unk4 = 0xFFFE7961;
    phi_v1->unk4 = 0;
    phi_v0->unk8 = 0xFFFE7961;
    phi_v1->unk8 = 0;
    phi_v0->unkC = 0xFFFE7961;
    phi_v1->unkC = 0;
    phi_v0 = phi_v0 + 0x10;
    phi_v1 = phi_v1 + 0x10;
    phi_a1 = temp_a1;
    if (temp_a1 != 0x40) {
        goto loop_2;
    }
    temp_a3 = phi_a3 + 0x100;
    phi_t0 = phi_t0 + 4;
    phi_t1 = phi_t1 + 4;
    phi_t2 = phi_t2 + 4;
    phi_a3 = temp_a3;
    phi_t3 = phi_t3 + 0x100;
    if (temp_a3 != &D_800F5770) {
        goto loop_1;
    }
    sp18 = func_800AEC70_ovl1(0x26, 0x4A, 0x50, temp_a3);
    if (func_800F8560_ovl1() != 9) {
        func_800BDFB8_ovl1(&D_800D5310, ((D_800D6BB0 * 4) + D_800D6BB0) * 2, 8);
    } else {
        D_800D6E54 = 0;
        D_800D6E90 = 0;
        func_800BDFB8_ovl1(&D_800D5310 + 0x5310, ((D_800D6BB0 * 4) + D_800D6BB0) * 2, 0xA);
    }
    func_800BDFB8_ovl1(&D_800D53DC, D_800D6BB0 * 2, 2);
    func_800BDFB8_ovl1(&D_800D5408, D_800D6BB0 * 2, 2);
    func_800BDFB8_ovl1(&D_800D5434, D_800D6BB0 * 2, 2);
    func_800BDFB8_ovl1(&D_800D5460, ((D_800D6BB0 * 4) + D_800D6BB0) * 2, 0xA);
    func_800A8934_ovl1(0x50001, 0, 0x10, &D_800ED500);
    D_800ED500.unk8 = (void *) (D_800ED500.unk8 + &D_800ED510);
    D_800ED500.unkC = (void *) (D_800ED500.unkC + &D_800ED510);
    D_800F6198 = 0;
    D_800D6EC4 = 0;
    if ((D_800D6F3C == 4) || (D_800D6F3C == 3)) {
        D_800F6198 = 1;
        temp_v0 = func_800AEA64_ovl1(0x2D, 0x4A, 0x50, &D_800ED500);
        temp_at = &D_800E98E0 + (temp_v0 * 4);
        sp18 = temp_v0;
        *temp_at = 0;
    } else {

    }
    func_800BDF2C_ovl1();
    func_800BDE0C_ovl1(0);
    func_80002B7C_ovl1(&func_800BDE0C);
    return sp18;
}
#else
GLOBAL_ASM("asm/non_matchings/ovl1_13/ovl1_13/func_800BE098.s")
#endif

#ifdef MIPS_TO_C
//generated by mips_to_c commit e0e006e8858ba357d1dcb4dc64f038b7df278aa6
? func_800BE320(void *arg0) {
    arg0->unkC = (?32) saved_reg_ra;
    arg0->unk68 = (?32) ERROR(cfc1);
    arg0->unk8 = (void *) sp;
    arg0->unk34 = (?32) saved_reg_fp;
    arg0->unk14 = (?32) saved_reg_s0;
    arg0->unk18 = (?32) saved_reg_s1;
    arg0->unk1C = (?32) saved_reg_s2;
    arg0->unk20 = (?32) saved_reg_s3;
    arg0->unk24 = (?32) saved_reg_s4;
    arg0->unk28 = (?32) saved_reg_s5;
    arg0->unk2C = (?32) saved_reg_s6;
    arg0->unk30 = (?32) saved_reg_s7;
    arg0->unk38 = (f64) saved_reg_f20;
    arg0->unk40 = (f64) saved_reg_f22;
    arg0->unk48 = (f64) saved_reg_f24;
    arg0->unk50 = (f64) saved_reg_f26;
    arg0->unk58 = (f64) saved_reg_f28;
    arg0->unk60 = (f64) saved_reg_f30;
    return 0;
}
#else
GLOBAL_ASM("asm/non_matchings/ovl1_13/ovl1_13/func_800BE320.s")
#endif

#ifdef MIPS_TO_C
//generated by mips_to_c commit e0e006e8858ba357d1dcb4dc64f038b7df278aa6
s32 func_800BE374(void *arg0, s32 arg1) {
    s32 phi_return;

    phi_return = arg1;
    if (arg1 == 0) {
        phi_return = 1;
    }
    return phi_return;
}
#else
GLOBAL_ASM("asm/non_matchings/ovl1_13/ovl1_13/func_800BE374.s")
#endif
