...
void crash_screen_draw_glyph(s32 x, s32 y, s32 glyph) {
    ...
    // data = D_8003F70C[glyph / 6 * 7];
    // // ptr = (osViGetCurrentFramebuffer() + ((gCurrScreenWidth * y) * 2)) + (x * 2);
    // ptr = (u32)osViGetCurrentFramebuffer() + (gCurrScreenWidth * y + x);
    data = &D_8003F70C[glyph / 5 * 7];
    ptr = (u32)osViGetCurrentFramebuffer() + gCurrScreenWidth * y + x;
    // ptr = osViGetCurrentFramebuffer() + gCrashScreen.width * (y) + (x);

    for (i = 0; i < 7; i++) {
        // bit = (0x80000000UL) >> ((s32)((glyph % 5) * 6) / (s32)2);
        // rowMask = (s32) ((glyph % 5) * 6);
        bit = 0x80000000UL >> (((glyph % 5) * 6) / ((s32) 2));
        rowMask = *data++;
...
void func_800231F0(void (*arg0)(void)) {
    s32 sp24;
    s32 temp_s0;
