...
def MHwrite(file,col):
    if col[-1]:
        ...
        file.write("\nstruct LevelHeader bank_%d_index_%d_level_header = { \n" %(globBank, globIndex) +
            TAB + "/* 0x0 */ %s,\n"%symbols[col[0][0][0]] +
            TAB + "/* 0x4 */ %s,\n"%symbols[col[0][1][0]] +
            TAB + "/* 0x8 */ %s,\n"% symbols[col[0][2][0]] +
            TAB + "/* 0xC */ %s"% "%d"% col[0][3][0]+"\n};\n\n")
...
def Vwrite(file,col):
    file.write("Vertex bank_%d_index_%d_vtx_%x[] = {\n" %(globBank, globIndex, col[1][2][1][0]))
...
def Triwrite(file,col):
    file.write("struct CollisionTriangle bank_%d_index_%d_collision_%X[] = {\n"%(globBank, globIndex, col[1][0][1][0]))
        ...
        if N[0][6][1][0]:
            ...
            for i,q in enumerate(N[4]):
                ...
                if i+offset == len(N[4]):
                    ...
                    file.write(" "+str(q)+",\n")
        file.write("struct PathNodeFooter bank_%d_index_%d_path_node_footer_%X = {\n"%(globBank, globIndex,N[3][1][1][0]))
        for i,q in enumerate(zip(N[0],Path_Footer.keys())):
            file.write(TAB)
            if q[1]==4 or q[1]==12:
                file.write("/*0x%X*/ "%q[1]+ "%ff"%q[0][1][0]+",\n")
            elif q[1] == 8:
                if q[0][1][0] == 0:
                    file.write("/*0x%02X*/ "%q[1]+ "NULL"+",\n")
                else:
                    file.write("/*0x%02X*/ "%q[1]+ "%s"%symbols[q[0][1][0]]+",\n")
            elif i+1 == len(N[0]):
