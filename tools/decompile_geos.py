...
if "--clean" in ''.join(sys.argv):
	subprocess.run("rm assets/geo/ -r", shell=True)
	subprocess.run("rm assets/image/ -r", shell=True)
	subprocess.run("rm assets/anim/ -r", shell=True)
	subprocess.run("rm assets/misc/ -r", shell=True)
	exit(0)
